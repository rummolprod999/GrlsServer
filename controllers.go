...
func indexHandler(w http.ResponseWriter, r *http.Request) {
	data := "API GRLS"
	tmpl, _ := template.New("data").Parse("<h1>{{ .}}</h1>Примеры:<p>GET /grls - возвращает весь список<p>GET /except - возвращает список исключенных<p>GET /grls/{штрих-код} - возвращает список по штрих-коду<p>GET /except/{штрих-код} - возвращает список исключенных по штрихкоду<p>POST /grlslist arr[0] = {code}, до 20 элементов<p>POST /exceptlist arr[0] = {code}, до 20 элементов<p>GET /dateup - дата последнего обновления списка грлс<p>GET /updatedb/{пароль}/{имя файла} - обновление базы<p>Все исполняемые файлы должны находиться в одном каталоге. При запуске сервера можно установить первым параметром порт(по умолчанию работает на 8181) и пароль для обновления базы - вторым параметром. Пароль можно установить только если указан порт.")
	tmpl.Execute(w, data)
}
